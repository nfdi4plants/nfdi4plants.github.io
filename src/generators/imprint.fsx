...
let createHero (hero:Imprintloader.ImprintHero) =
...
let createCard (card:Imprintloader.ImprintCard) =
...
let generate' (ctx : SiteContents) (page: string) =
    let ImprintPages = ctx.TryGetValues<Imprintloader.ImprintPage>() |> Option.defaultValue Seq.empty

    printfn "[Imprint-Generator] mapping imprint page for %s" (page.Replace("/hero.md",""))
    
    let currentImprintPage =
        ImprintPages
        |> Seq.tryFind (fun lmp ->
            lmp.OutputName = System.IO.DirectoryInfo(page.Replace("/hero.md","")).Name
        )

    match currentImprintPage with
    | Some page ->
        Layout.layout ctx "" [
    ...
    | None -> 
        printfn "[Imprint-Generator] No imprint page found for %s" page
        div [] []
