...
let createHero (hero:Datahubloader.DatahubHero) =
...
let createCard (card:Datahubloader.DatahubCard) =
...
let generate' (ctx : SiteContents) (page: string) =
    let DatahubPages = ctx.TryGetValues<Datahubloader.DatahubPage>() |> Option.defaultValue Seq.empty

    printfn "[Datahub-Generator] mapping datahub page for %s" (page.Replace("/hero.md",""))
    
    let currentDatahubPage =
        DatahubPages
        |> Seq.tryFind (fun lmp ->
            lmp.OutputName = System.IO.DirectoryInfo(page.Replace("/hero.md","")).Name
        )

    match currentDatahubPage with
    | Some page ->
        Layout.layout ctx "" [
    ...
    | None -> 
        printfn "[Datahub-Generator] No datahub page found for %s" page
        div [] []
