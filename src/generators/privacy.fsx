...
let createHero (hero:Privacyloader.PrivacyHero) =
...
let createCard (card:Privacyloader.PrivacyCard) =
...
let generate' (ctx : SiteContents) (page: string) =
    let PrivacyPages = ctx.TryGetValues<Privacyloader.PrivacyPage>() |> Option.defaultValue Seq.empty

    printfn "[Privacy-Generator] mapping Privacy page for %s" (page.Replace("/hero.md",""))
    
    let currentPrivacyPage =
        PrivacyPages
        |> Seq.tryFind (fun lmp ->
            lmp.OutputName = System.IO.DirectoryInfo(page.Replace("/hero.md","")).Name
        )

    match currentPrivacyPage with
    | Some page ->
        Layout.layout ctx "" [
    ...
    | None -> 
        printfn "[Privacy-Generator] No Privacy page found for %s" page
        div [] []
