...
type PrivacyHero = {
    ...
    static member fromFile heroMarkdownPath = 
        ...
        PrivacyHero.create heading title bgColor emphasisColor image layout body 
...
type PrivacyCard = {
    ...
    static member fromFile heroMarkdownPath = 
        ...
        PrivacyCard.create title bgColor borderColor emphasisColor image layout index body 
...
type PrivacyPage = {
    ...
    static member fromFolder (rootDir:string) (folderPath:string) =
        ...
        let hero = 
            ...
            |> Option.map PrivacyHero.fromFile

        let cards = 
            content 
            |> Array.filter (fun c -> Path.GetFileNameWithoutExtension c <> "hero")
            |> Array.map PrivacyCard.fromFile

        match hero with
        | Some h -> 
            printfn "[Privacy-Loader]: Found hero at %s" folderPath
            PrivacyPage.create relPath outName h cards
        | None -> failwithf "[Privacy-Loader] directory %s does not contain a hero.md file." folderPath


let contentDir = "content/privacy"

let loader (projectRoot: string) (siteContent: SiteContents) =
    let PrivacyPath = System.IO.Path.Combine(projectRoot, contentDir)
    try 
        siteContent.Add (PrivacyPage.fromFolder projectRoot PrivacyPath)
    with e as exn -> 
        siteContent.AddError {Path = PrivacyPath; Message = (sprintf "Unable to load mainpagecard %s. \n%s" PrivacyPath e.Message); Phase = Loading}

    printfn "[Privacy-Loader]: Done loading Privacy pages"
    siteContent